...
class ClayWorkTableRecipeWrapper(val recipe: ClayWorkTableRecipe) : IRecipeWrapper {
    ...
    override fun getIngredients(ingredients: IIngredients) {
        ingredients.setInputLists(
            VanillaTypes.ITEM,
            listOf(recipe.input.inputStacks, recipe.method.requiredTools.map { ItemStack(it) })
        )
        ...
        for (id in ClayWorkTableMethod.ids) {
            if (id == recipe.method.id) {
                ...
                minecraft.fontRenderer.drawString(
                    cStr,
                    41 + 16 * id - 5 + 8 - minecraft.fontRenderer.getStringWidth(cStr) / 2,
                    36,
                    0
                )
        ...
        private val buttons =
            listOf(
                    List(ClayWorkTableMethod.entries.size) { i ->
                        if (i == 5) guiHelper.createDrawable(GUI_IMAGE, 176, 80, 16, 16)
                        else guiHelper.createDrawable(GUI_IMAGE, 176 + i * 16, 32, 16, 16)
                    },
                    List(ClayWorkTableMethod.entries.size) { i ->
                        if (i == 5) guiHelper.createDrawable(GUI_IMAGE, 176, 96, 16, 16)
                        else guiHelper.createDrawable(GUI_IMAGE, 176 + i * 16, 48, 16, 16)
                    }
                )
                .flatten()
    }
