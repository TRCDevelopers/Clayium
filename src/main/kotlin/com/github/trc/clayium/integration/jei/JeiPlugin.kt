...
class JeiPlugin : IModPlugin {
    ...
    override fun register(modRegistry: IModRegistry) {
        ...
        modRegistry.handleRecipes(
            ClayWorkTableRecipe::class.java,
            ::ClayWorkTableRecipeWrapper,
            ClayWorkTableRecipeCategory.UID
        )
        modRegistry.addRecipeCatalyst(
        ...
        modRegistry.addRecipeClickArea(
            GuiClayWorkTable::class.java,
            78,
            29,
            20,
            16,
        ...
        for (recipeRegistry in CRecipes.ALL_REGISTRIES.values) {
            ...
            if (specialWrapper != null) {
                ...
                modRegistry.handleRecipes(
                    MetalSeparatorRecipeWrapper.RecipeData::class.java,
                    ::MetalSeparatorRecipeWrapper,
                    recipeRegistry.category.uniqueId
                )
                modRegistry.addRecipes(
                    recipeRegistry
                        ...
                        .flatMap {
                            it.chancedOutputs!!.chancedOutputs.mapIndexed { i, _ ->
                                MetalSeparatorRecipeWrapper.RecipeData(it, i)
                            }
                        },
                    recipeRegistry.category.uniqueId
                )
        ...
        modRegistry.addIngredientInfo(
            listOf(
                ItemStack(ClayiumBlocks.CLAY_TREE_SAPLING),
                ItemStack(ClayiumBlocks.CLAY_TREE_LOG),
                ItemStack(ClayiumBlocks.CLAY_TREE_LEAVES)
            ),
            VanillaTypes.ITEM,
            "recipe.clayium.clay_tree.description"
        )
        modRegistry.addIngredientInfo(
            ItemStack(ClayiumBlocks.QUARTZ_CRUCIBLE),
            VanillaTypes.ITEM,
            "recipe.clayium.quartz_crucible.description"
        )
        modRegistry.addIngredientInfo(
            OreDictUnifier.get(OrePrefix.gem, CMaterials.pureAntimatter),
            VanillaTypes.ITEM,
    ...
    companion object {
        ...
        fun registerWrapper(
            ...
            wrapperFactory: IRecipeWrapperFactory<Recipe>
        ) {
