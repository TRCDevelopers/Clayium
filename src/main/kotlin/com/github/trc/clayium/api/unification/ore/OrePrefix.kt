...
class OrePrefix(
    ...
    private val modifiedAmounts =
        Object2LongOpenHashMap<IMaterial>().apply { defaultReturnValue(FALLBACK.toLong()) }
    ...
    fun canGenerateItem(material: CMaterial): Boolean {
        return !this.isIgnored(material) &&
            (itemGenerationLogic != null && itemGenerationLogic.test(material))
    }
    ...
    fun getLocalizedName(material: CMaterial): String {
        ...
        return BothSideI18n.format(
            "${MOD_ID}.ore_prefix.${snake}",
            BothSideI18n.format(material.translationKey)
        )
    }
    ...
    companion object {
        ...
        private val hasIngotProperty =
            Predicate<CMaterial> { it.hasProperty(CPropertyKey.Companion.INGOT) }
        private val hasDustProperty =
            Predicate<CMaterial> { it.hasProperty(CPropertyKey.Companion.DUST) }
        private val hasPlateProperty =
            Predicate<CMaterial> { it.hasProperty(CPropertyKey.Companion.PLATE) }
        private val hasMatterProperty =
            Predicate<CMaterial> { it.hasProperty(CPropertyKey.Companion.MATTER) }
        private val hasImpureDustProperty =
            Predicate<CMaterial> { it.hasProperty(CPropertyKey.Companion.IMPURE_DUST) }
        private val hasClayPartsFlag =
            Predicate<CMaterial> { it.hasFlag(CMaterialFlags.GENERATE_CLAY_PARTS) }
        ...
        fun init() {
            ...
            listOf(
                    CMaterials.pureAntimatter1,
                    CMaterials.pureAntimatter2,
                    CMaterials.pureAntimatter3,
                    CMaterials.pureAntimatter4,
                    CMaterials.pureAntimatter5,
                    CMaterials.pureAntimatter6,
                    CMaterials.pureAntimatter7
                )
                .forEach { block.ignore(it) }
