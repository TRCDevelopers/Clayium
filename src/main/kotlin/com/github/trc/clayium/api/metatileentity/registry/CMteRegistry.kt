...
class CMteRegistry(
    ...
    val blockMachine by lazy {
        if (!Loader.instance().hasReachedState(LoaderState.PREINITIALIZATION))
            throw IllegalStateException("Too early to access blockMachine")
        BlockMachine().apply {
            // todo: datafix?
            registryName =
                if (modId == MOD_ID) clayiumId("machine")
                else ResourceLocation(modId, "clayium_machine")
        }
    }

    val itemBlockMachine by lazy {
        ItemBlockMachine(blockMachine).apply {
            registryName =
                if (modId == MOD_ID) clayiumId("machine")
                else ResourceLocation(modId, "clayium_machine")
        }
    ...
    override fun register(id: Int, key: ResourceLocation, value: MetaTileEntity) {
        if (key.namespace != modId) {
            throw IllegalArgumentException(
                "Cannot register a MetaTileEntity to another mod's registry"
            )
        }
