...
open class RecipeLogicEnergy(
    ...
    override fun drawEnergy(ce: ClayEnergy, simulate: Boolean): Boolean {
        return energyHolder.drawEnergy(
            ClayEnergy((ce.energy * ocHandler.accelerationFactor).toLong()),
            simulate
        )
    }
    ...
    override fun applyOverclock(
        ...
        return longArrayOf(
            (cet * energyConsumingMultiplier).toLong(),
            (duration * durationMultiplier).toLong()
        )
    }

    /**
     * Mainly used by Condensers, Grinders, Centrifuges. The speed of these machines depends on the
     * tier.
     */
    ...
    override fun addProbeInfo(
        ...
        if (this.isWorking) {
            ...
            probeInfo.text(
                "Using ${TextFormatting.RED}${cet.formatWithoutUnit()}${TextFormatting.WHITE} CE/t"
            )
        }
    }
}
