...
class MultiTrackBufferMetaTileEntity(
    ...
    MetaTileEntity(
        metaTileEntityId,
        tier,
        validInputModes = mBufferValidIoModes[getTrackRows(tier.numeric) - 2],
        validOutputModes = mBufferValidIoModes[getTrackRows(tier.numeric) - 2],
        "multi_track_buffer"
    ) {
    ...
    val tracks: Array<ClayiumItemStackHandler> =
        (0..<trackRow).map { ClayiumItemStackHandler(this, trackInvSize) }.toTypedArray()

    private val filtersHandler = ClayiumItemStackHandler(this, trackRow)
    private val slotFilters =
        (0..<filtersHandler.slots).map { slot ->
            { stack: ItemStack ->
                ...
                else if (filter != null) filter.test(stack) else filterStack.isItemEqual(stack)
            }
        }
    ...
    override fun <T> getCapability(capability: Capability<T>, facing: EnumFacing?): T? {
        if (capability === CapabilityItemHandler.ITEM_HANDLER_CAPABILITY) {
            ...
            return CapabilityItemHandler.ITEM_HANDLER_CAPABILITY.cast(
                ItemHandlerProxy(
                    getItemHandler(getInput(facing), facing),
                    getItemHandler(getOutput(facing), facing)
                )
            )
        }
    ...
    private fun getTrackWithFilter(track: Int): IItemHandler =
        FilteredItemHandler(tracks[track], slotFilters[track])
    ...
    override fun buildUI(data: MetaTileEntityGuiData, syncManager: GuiSyncManager): ModularPanel {
        ...
        return ModularPanel("multi_track_buffer")
            .flex {
                it.size(
                    max(GUI_DEFAULT_WIDTH, trackInvSize * 18 + 4 + 18 + /* margin*/ 12),
                    18 + trackRow * 18 + 94 + 2
                )
                                        ...
                                        column.child(
                                            Row()
                                                    ...
                                                    SlotGroupWidget.builder()
                                                        ...
                                                        .key('I') { slotIndex ->
                                                            ItemSlot()
                                                                ...
                                                                .background(
                                                                    ClayGuiTextures.M_TRACK_SLOTS[i]
                                                                )
                                                        }
                                                        .build()
                                                )
                                                .child(
                                                    ItemSlot()
                                                        .slot(
                                                            SyncHandlers.phantomItemSlot(
                                                                filtersHandler,
                                                                i
                                                            )
                                                        )
                                                        .background(
                                                            ClayGuiTextures.M_TRACK_FILTER_SLOTS[i]
                                                        )
    ...
    private inner class MultiTrackIoHandler :
        AutoIoHandler.Combined(this@MultiTrackBufferMetaTileEntity, isBuffer = true) {
        override fun importFromNeighbors() {
            ...
            for (side in EnumFacing.entries) {
                ...
                if (
                    neighbor is MetaTileEntityHolder &&
                        neighbor.metaTileEntity is MultiTrackBufferMetaTileEntity
                ) {
                    val neighborBuffer = neighbor.metaTileEntity as MultiTrackBufferMetaTileEntity
                    remaining =
                        transferMultiTrack(
                            neighborBuffer,
                            this@MultiTrackBufferMetaTileEntity,
                            remaining
                        )
                } else {
                    remaining =
