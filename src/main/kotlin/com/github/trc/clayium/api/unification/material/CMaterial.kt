...
class CMaterial(
    ...
    override val upperCamelName: String =
        CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, materialId.path)
    ...
    class Builder(
        ...
        fun colors(vararg colors: Int): Builder {
            if (colors.isEmpty()) {
                CLog.warn(
                    "Material.Builder#colors is called, but provided array is empty. Ignoring."
                )
                return this
            }
            this.colors = colors
        ...
        fun impureDust(color1: Int, color2: Int, color3: Int): Builder {
            properties.setProperty(
                CPropertyKey.IMPURE_DUST,
                MaterialProperty.ImpureDust(color1, color2, color3)
            )
            return this
        }

        /**
         * Adds a plate item to this material. If the material has an ingot, dust, or block
         * property, the plate recipe will be generated with the given parameters.
         */
        fun plate(cePerTick: ClayEnergy, requiredTick: Int, tier: Int): Builder {
            properties.setProperty(
                CPropertyKey.PLATE,
                MaterialProperty.Plate(cePerTick, requiredTick, tier)
            )
            return this
        }

        /**
         * Adds a clay block to this material. automatically set blockAmount to 1.
         *
         * @param compressedInto If specified, the compress/condense and inverse recipe will be
         *   generated.
         * @param energy The energy of this clay. If null, the clay will not be energized (i.e. it
         *   can't be used as machine fuel).
         */
        fun clay(
            compressionLevel: Int,
            compressedInto: CMaterial? = null,
            energy: ClayEnergy? = null
        ): Builder {
            properties.setProperty(
                CPropertyKey.CLAY,
                Clay(compressionLevel, compressedInto, energy)
            )
            blockAmount(1)
            return this
        }
        ...
        fun blastSmelting(factor: Double, tier: Int, duration: Int): Builder {
            properties.setProperty(
                CPropertyKey.BLAST_SMELTING,
                BlastSmelting(factor, tier, duration)
            )
            return this
        }
