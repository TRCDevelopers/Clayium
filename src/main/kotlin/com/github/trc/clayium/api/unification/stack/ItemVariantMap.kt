...
interface ItemVariantMap<E> {
    val isEmpty: Boolean get() = !hasWildcardEntry() && !hasNonWildcardEntry()

    /**
     * @param meta Item metadata
     * @return `true` if there's a nonnull value associated with given item metadata, `false` otherwise.
     */
    ...
    /**
     * @return `true` if there's any nonnull value associated with some item metadata, excluding metadata of [net.minecraftforge.oredict.OreDictionary.WILDCARD_VALUE] `(32767)`.
     */
    fun hasNonWildcardEntry(): Boolean

    /**
     * @return `true` if there's a nonnull value associated with metadata of [net.minecraftforge.oredict.OreDictionary.WILDCARD_VALUE] `(32767)`.
     */
