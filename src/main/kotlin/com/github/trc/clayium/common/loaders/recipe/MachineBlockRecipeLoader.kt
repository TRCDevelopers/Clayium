...
object MachineBlockRecipeLoader {
    fun registerRecipes() {
            ...
            when (tier) {
                DEFAULT ->
                    RecipeUtils.addShapedRecipe(
                        ...
                        "PPP",
                        'P',
                        UnificationEntry(OrePrefix.largePlate, CMaterials.clay),
                        'G',
                        UnificationEntry(OrePrefix.gear, CMaterials.clay)
                    )
                ...
                SIMPLE ->
                    RecipeUtils.addShapedRecipe(
                        ...
                        circuits[i],
                        'P',
                        UnificationEntry(OrePrefix.largePlate, mainHullMaterials[i])
                    )
                BASIC,
                PRECISION,
                CLAY_STEEL,
                ...
                ADVANCED ->
                    RecipeUtils.addShapedRecipe(
                        ...
                        circuits[i],
                        'S',
                        UnificationEntry(OrePrefix.largePlate, CMaterials.silicone),
                        'P',
                        UnificationEntry(OrePrefix.largePlate, mainHullMaterials[i])
                    )
                AZ91D ->
                ...
                ZK60A ->
                    RecipeUtils.addShapedRecipe(
                        ...
                        MetaItemClayParts.PrecisionCircuit,
                        'P',
                        UnificationEntry(OrePrefix.largePlate, CMaterials.zk60a)
                    )
            }
        }
        /* CA Reactor Hulls */
        ...
        val caHullMaterials =
            listOf(
                ...
                CMaterials.cerium,
                CMaterials.tantalum,
                CMaterials.praseodymium,
                CMaterials.protactinium,
                CMaterials.neptunium,
                CMaterials.promethium,
                CMaterials.samarium,
                CMaterials.curium,
                CMaterials.europium
            )
        ...
        for ((i, entry) in antimatters.zip(caHullMaterials).withIndex()) {
            ...
            RecipeUtils.addShapedRecipe(
                ...
                "MMM",
                'M',
                UnificationEntry(OrePrefix.gem, matter),
                'I',
                UnificationEntry(OrePrefix.ingot, hullMaterial),
                'H',
                caHullCenter,
            )
            caHullCenter = result
        ...
        for ((i, entry) in
            ...
            CRecipes.CLAY_REACTOR.builder()
                ...
                .tier(10 + i)
                .duration(min(10_000_000_000_000 * 10.0.pow(i).toLong(), 1_000_000_000_000_000))
        ...
        registerLowTierRecipe(
            MetaTileEntities.ENERGETIC_CLAY_CONDENSER,
            "pgp",
            "EHE",
            "pcp",
            'E',
        ...
        for (i in 0..9) {
            ...
            CRecipes.ASSEMBLER.builder()
                ...
                .tier(4)
                .CEt(ClayEnergy(10.0.pow((i + 1.0)).toLong()))
                .duration(40)
                .buildAndRegister()
            CRecipes.ASSEMBLER.builder()
        ...
        for (metaTileEntity in MetaTileEntities.CLAY_INTERFACE) {
            CRecipes.ASSEMBLER.builder()
                ...
                .tier(4)
                .CEt(ClayEnergy(10.0.pow(metaTileEntity.tier.numeric - 3).toLong()))
                .duration(40)
                .buildAndRegister()
        }
        /* Redstone Proxy */
        ...
        for ((i, m) in
            listOf(CMaterials.clay, CMaterials.denseClay, CMaterials.industrialClay).withIndex()) {
            assembler
                .builder()
        ...
        RecipeUtils.addShapelessRecipe(
            "upgrade_storage_container",
            MetaTileEntities.STORAGE_CONTAINER_UPGRADED.getStackForm(),
            MetaTileEntities.STORAGE_CONTAINER,
            MetaItemClayParts.ClayCore
        )
        ...
        for (i in 0..<3) {
            ...
            assembler
                ...
                .tier(6)
                .CEt(ClayEnergy.of(1 * 10.0.pow(i).toLong()))
                .duration(120)
                .buildAndRegister()
        }
        ...
        val wheels =
            listOf(
                UnificationEntry(OrePrefix.wheel, CMaterials.clay),
                UnificationEntry(OrePrefix.wheel, CMaterials.denseClay)
            )
        for ((i, e) in wheelHulls.zip(wheels).withIndex()) {
            val (block, item) = e
            RecipeUtils.addShapelessRecipe(
                "$MOD_ID.waterwheel_$i",
                MetaTileEntities.WATERWHEEL[i].getStackForm(),
                block,
                item
            )
        }
        ...
        for ((i, mte) in MetaTileEntities.MATTER_TRANSFORMER.withIndex()) {
            if (i in 0..2) {
                assembler
                    ...
                    .output(mte)
                    .tier(6)
                    .CEt(ClayEnergy.of(10.0.pow(i).toLong()))
                    .duration(120)
                    .buildAndRegister()
            } else {
                assembler
        ...
        CRecipes.CLAY_REACTOR.builder()
            ...
            .tier(13)
            .CEt(ClayEnergy.of(10_000_000))
            .duration(100_000_000_000_000_000)
            .buildAndRegister()
        ...
        for ((i, type) in BlockCaReactorCoil.BlockType.entries.withIndex()) {
            CRecipes.ASSEMBLER.builder()
                ...
                .tier(10)
                .CEt(ClayEnergy(10.0.pow((i + 9.0)).toLong()))
                .duration(60)
                .buildAndRegister()
        }
        ...
        RecipeUtils.addShapedRecipe(
            "chunk_loader",
            ItemStack(ClayiumBlocks.CHUNK_LOADER),
            "cCc",
            "CHC",
            "cCc",
            'c',
            ...
            MetaItemClayParts.PrecisionCircuit,
            'H',
            MACHINE_HULL.getItem(ZK60A)
        )
    }

    /**
     * Register workbench recipes for tier <= 4 machines. Clay is [CMaterials.clay] if tier is 1,
     * [CMaterials.denseClay] otherwise. Some characters are available for recipes by default:
     * - 'H' for machine hull
    ...
    private fun registerLowTierRecipe(metaTileEntities: List<MetaTileEntity>, vararg recipe: Any) {
        ...
        for (mte in metaTileEntities) {
            ...
            val ingMap =
                mutableMapOf(
                    'p' to UnificationEntry(OrePrefix.plate, clay),
                    'P' to UnificationEntry(OrePrefix.largePlate, clay),
                    'g' to UnificationEntry(OrePrefix.gear, clay),
                    'G' to UnificationEntry(OrePrefix.cuttingHead, clay),
                    's' to UnificationEntry(OrePrefix.stick, clay),
                    'S' to UnificationEntry(OrePrefix.stick, clay),
                    'M' to UnificationEntry(OrePrefix.spindle, clay),
                    'c' to circuits[mte.tier.numeric],
                    'C' to UnificationEntry(OrePrefix.cuttingHead, clay),
                    'O' to UnificationEntry(OrePrefix.pipe, clay),
                    'I' to UnificationEntry(OrePrefix.cylinder, clay),
                )
            ...
            RecipeUtils.addShapedRecipe(
                "${mte.metaTileEntityId}.workbench",
                mte.getStackForm(),
                *recipe,
                'H',
                MACHINE_HULL.getItem(mte.tier),
                *recipeExtra.toTypedArray()
            )
        }
    }

    private fun registerMachineRecipeHull(
        metaTileEntities: List<MetaTileEntity>,
        inputProvider: RecipeBuilder<*>.(index: Int) -> RecipeBuilder<*>
    ) {
        for ((i, mte) in metaTileEntities.withIndex()) {
            if (mte.tier !is ClayTiers) {
                CLog.warn(
                    "MetaTileEntity ${mte.metaTileEntityId}'s tier is not a instance of ClayTiers. Cannot register recipe."
                )
                continue
            }
            CRecipes.ASSEMBLER.builder()
    ...
    private fun registerMachineRecipeBuffer(
        metaTileEntities: List<MetaTileEntity>,
        inputProvider: RecipeBuilder<*>.() -> RecipeBuilder<*>
    ) {
        for (mte in metaTileEntities) {
            if (!(mte.tier is ClayTiers && mte.tier.numeric >= 4 && mte.tier.numeric <= 13))
                continue
    ...
    private fun panDuplicators() {
        ...
        for ((i, duplicator) in MetaTileEntities.PAN_DUPLICATOR.slice(1..9).withIndex()) {
            RecipeUtils.addShapedRecipe(
                "pan_duplicator_rank${i + 1}",
                duplicator.getStackForm(),
                "PIP",
                "DHD",
                "PIP",
                'P',
                UnificationEntry(OrePrefix.gem, CMaterials.PURE_ANTIMATTERS[i]),
                'I',
                UnificationEntry(OrePrefix.ingot, ingotMaterials[i]),
                'D',
                MetaTileEntities.PAN_DUPLICATOR[i].getStackForm(),
                'H',
                MACHINE_HULL.getItem(ClayTiers.entries[i + 5])
            )
        }
    }
}
