 ...
 *
 * Output is automatically set if input material has clay property or can be set manually. Duration
 * is calculated by [requiredTicksCalculator], or can be set manually. [cePerTick] is automatically
 * calculated.
 */
class ClayFabricatorRecipeBuilder : RecipeBuilder<ClayFabricatorRecipeBuilder> {
    ...
    override fun buildAndRegister() {
        val duration =
            if (duration == 0L) requiredTicksCalculator(this.tier, inputTier) else duration

        val outputEnergy =
            outputs[0]
                .getCapability(ClayiumCapabilities.ENERGIZED_CLAY, null)
                ?.getClayEnergy()
                ?.energy ?: 0
        val inputEnergy =
            inputs[0]
                ...
                .firstNotNullOfOrNull {
                    it.getCapability(ClayiumCapabilities.ENERGIZED_CLAY, null)?.getClayEnergy()
                }
                ?.energy ?: 0
