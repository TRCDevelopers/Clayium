...
class RecipeInput(
    ...
    // [Ingredient.matchingStacks] returns an empty list in [OreIngredient], so use
    // [Ingredient.getMatchingStacks] instead.
    val inputStacks =
        stacks.getMatchingStacks().map {
            it.copy().apply { this@apply.count = this@RecipeInput.amount }
        }
