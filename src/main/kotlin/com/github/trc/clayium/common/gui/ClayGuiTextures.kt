...
object ClayGuiTextures {
    ...
    val LARGE_SLOT = slotTexture().uv(0, 32, 26, 26).canApplyTheme().build()

    val CLAY_SLOT = slotTexture().uv(96, 0, 18, 18).build()

    val FILTER_SLOT = slotTexture().uv(96, 32, 18, 18).build()

    // memo: gui/slot slot interval is 14 pixels
    val IMPORT_1_SLOT = slotTexture().uv(32, 0, 18, 18).build()
    val IMPORT_2_SLOT = slotTexture().uv(32, 32, 18, 18).build()

    val EXPORT_1_SLOT = slotTexture().uv(64, 0, 18, 18).build()
    val EXPORT_2_SLOT = slotTexture().uv(64, 32, 18, 18).build()
    ...
    private fun slotTexture() = UITexture.builder().location(MOD_ID, "gui/slot").imageSize(256, 256)

    private fun button(u: Int, v: Int) =
        UITexture.builder()
