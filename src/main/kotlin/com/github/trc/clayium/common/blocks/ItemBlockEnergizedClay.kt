...
class ItemBlockEnergizedClay(
    block: BlockEnergizedClay,
    orePrefix: OrePrefix,
) : ItemBlockMaterial(block, orePrefix) {
        ...
        return object : ItemCapabilityProvider {
            override fun <T : Any> getCapability(capability: Capability<T?>): T? {
                if (capability == ClayiumCapabilities.ENERGIZED_CLAY) {
                    val energy =
                        blockMaterial.getCMaterial(stack).getPropOrNull(CPropertyKey.CLAY)?.energy
                    if (energy != null) {
                        return ClayiumCapabilities.ENERGIZED_CLAY.cast(
                            IClayEnergyProvider { energy }
                        )
                    }
                }
                return null
