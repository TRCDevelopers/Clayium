...
class MetaTileEntityPipeBakedModel : IModelParticleProvider {
    ...
    val cubeQuads: Map<ITier, List<List<BakedQuad>>> =
        ClayTiers.entries.associateWith { iTier ->
                    ...
                    .map { quadSide ->
                        ModelTextures.faceBakery.makeBakedQuad(
                            ...
                            BlockPartFace(
                                ...
                                0,
                                "",
                                getUv(cubePos = cubePos, sideOfCube = quadSide)
                            ),
    ...
    val centerCubeQuads: Map<ITier, List<BakedQuad>> =
        ClayTiers.entries.associateWith { tier ->
    ...
    override fun getQuads(state: IBlockState?, side: EnumFacing?, rand: Long): List<BakedQuad> {
        ...
        val metaTileEntity =
            (state.getValue(BlockMachine.TILE_ENTITY) as? MetaTileEntityHolder)?.metaTileEntity
                ?: return emptyList()
    ...
    private fun getUv(cubePos: EnumFacing, sideOfCube: EnumFacing): BlockFaceUV {
        ...
        return when (cubePos) {
            ...
            EnumFacing.WEST ->
                when (sideOfCube) {
                    EnumFacing.UP,
                    EnumFacing.DOWN -> BlockFaceUV(floatArrayOf(0f, 5f, 5f, 11f), 0)
            ...
            EnumFacing.EAST ->
                when (sideOfCube) {
                    EnumFacing.UP,
                    EnumFacing.DOWN -> BlockFaceUV(floatArrayOf(11f, 5f, 16f, 11f), 0)
