...
object ModelUtils {
    ...
    private val uvCacheInt: Map<EnumFacing, Map<EnumFacing, IntArray>> =
        EnumFacing.entries.associateWith { cubePos ->
            EnumFacing.entries
    ...
    private val uvCacheFloat: Map<EnumFacing, Map<EnumFacing, FloatArray>> =
        EnumFacing.entries.associateWith { cubePos ->
            EnumFacing.entries
                .filter { it != cubePos.opposite }
                .associateWith { facingOfCube -> getUv(cubePos, facingOfCube) }
        }
    ...
    fun createCenterCubeQuads(machineHull: TextureAtlasSprite): List<BakedQuad> {
        return EnumFacing.entries.map {
            createQuad(
                ...
                floatArrayOf(5f, 5f, 11f, 11f)
            )
        }
    }

    /**
     * if `sideOfCube == cubePos.opposite`, this throws IllegalArgumentException because that face
     * is always hided.
     */
    fun getUvInt(cubePos: EnumFacing, sideOfCube: EnumFacing): IntArray {
        return uvCacheInt[cubePos]?.get(sideOfCube)
            ?: throw IllegalArgumentException("Invisible side $sideOfCube of cube $cubePos")
    }
    ...
    fun getUvFloat(cubePos: EnumFacing, sideOfCube: EnumFacing): FloatArray {
        return uvCacheFloat[cubePos]?.get(sideOfCube)
            ?: throw IllegalArgumentException("Invisible side $sideOfCube of cube $cubePos")
    }
    ...
    private fun getUv(cubePos: EnumFacing, sideOfCube: EnumFacing): FloatArray {
        ...
        return when (cubePos) {
            ...
            EnumFacing.WEST ->
                when (sideOfCube) {
                    EnumFacing.UP,
                    EnumFacing.DOWN -> floatArrayOf(0f, 5f, 5f, 11f)
            ...
            EnumFacing.EAST ->
                when (sideOfCube) {
                    EnumFacing.UP,
                    EnumFacing.DOWN -> floatArrayOf(11f, 5f, 16f, 11f)
