...
enum class EnumMaterial(
    ...
    PHOSPHORUS(1010, "phosphorus", colors = intArrayOf(0xB4B419, 0x9B9B00, 0xCDCD32), properties = setOf(Dust)),
    QUARTZ(1011, "quartz", properties = setOf(Dust)),
    SALT(1012, "salt", properties = setOf(Dust)),
    SALTPETER(1013, "saltpeter", colors = intArrayOf(0xDEDCDC, 0xBEC8D2, 0xFFF0E6), properties = setOf(Dust)),
    SODIUM_CARBONATE(1014, "sodium_carbonate", properties = setOf(Dust)),
    SULFUR(1015, "sulfur", colors = intArrayOf(0xE6E600, 0xCDCD00, 0xFFFF00), properties = setOf(Dust)),
    ;

    val propertiess: Set<MaterialProperty> = run {
    ...
    inline fun <reified T : MaterialProperty> hasProperty(): Boolean {
        return propertiess.firstOrNull { it is T } != null
    }

    inline fun <reified T : MaterialProperty> getProperty(): T? {
        return propertiess.first { it is T } as? T
    }
    ...
    companion object {
        private val uniqueIdMap: IntIdentityHashBiMap<EnumMaterial> = run {
            val map = IntIdentityHashBiMap<EnumMaterial>(entries.size)
            entries.forEach {
        ...
        fun getId(material: EnumMaterial): Int {
            return uniqueIdMap.getId(material)
        }

        fun fromId(id: Int): EnumMaterial? {
