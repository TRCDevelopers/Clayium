package io.github.trcdevelopers.clayium.network

import io.github.trcdevelopers.clayium.Bootstrap
import io.github.trcdevelopers.clayium.api.network.PacketDataList
            ...
            for (discriminator in entry.keySet) {
                when (i) {
                    0 -> {
                        ...
                        entry.getByteArray(discriminator).toList() shouldContainAll byteArrayOf(0, 1, 2, 3).toList()
                    }
                    1 -> {
                        discriminator.toInt() shouldBe 1
                        entry.getByteArray(discriminator).toList() shouldContainAll byteArrayOf(4, 5, 6, 7).toList()
                    }
                    2 -> {
                        discriminator.toInt() shouldBe 1
                        entry.getByteArray(discriminator).toList() shouldContainAll byteArrayOf(1).toList()
                    }
                }
            }
        }
